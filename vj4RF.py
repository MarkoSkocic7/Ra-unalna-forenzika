...
def figMetaData(file_path):
    ...
    print(f"{dir(img_doc)}\n")
    decimalLongitude = convertGPScoordinate(
        img_doc.gps_longitude, img_doc.gps_longitude_ref
    )
    decimalLatitude = convertGPScoordinate(
        img_doc.gps_latitude, img_doc.gps_latitude_ref
    )
    print(f"Longitude: {decimalLongitude}, Latitude: {decimalLatitude}")
    url = f"http://www.google.com/maps/place/{decimalLatitude,decimalLongitude}"
